...
BEGIN
    ...
    LOOP
        ...
        RETURNING id INTO current_hashtag_id;

        -- Link hashtag to tweet using NEW.id
        IF current_hashtag_id IS NOT NULL THEN
            INSERT INTO public.tweet_hashtags (tweet_id, hashtag_id)
            VALUES (NEW.id, current_hashtag_id)
...
-- Update hashtag counts view with explicit table aliases
